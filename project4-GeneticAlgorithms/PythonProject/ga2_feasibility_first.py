...
# penalty only for power (SINR handled by repair)
LAMBDA_PWR = 50.0
...
def sensing_snr(p):
    # sensing streams are the last Q columns
...
# =====================================================
# FEASIBILITY-FIRST REPAIR (KEY OF GA2)
# =====================================================
def repair_feasible(p, max_iter=20):
    """
    Feasibility-first repair:
    1) Reduce sensing power if SINR violated
    2) Reallocate freed power to communication
    """
    p = repair_power(p)
    ...
    for _ in range(max_iter):
        ...
        # Step 1: reduce sensing power
        p[:, U:] *= 0.7   # aggressively cut sensing

        # Step 2: redistribute to communication users
        for u in range(U):
            if sinr[u] < gamma:
                p[:, u] += 0.1 * P_AP / U

        # Step 3: re-normalize per AP
        p = repair_power(p)
...
# =====================================================
# FITNESS FUNCTION — GA2
# =====================================================
def fitness_ga2(p):
    sinr = compute_sinr(p)

    # Step 1: infeasible solutions are heavily penalized
    if np.any(sinr < gamma):
        return -1e6 - np.sum(np.maximum(0, gamma - sinr)) * 1e3

    # Step 2: only feasible solutions optimize sensing
...
# =====================================================
# MONTE CARLO EVALUATION — GA2
# =====================================================
if __name__ == "__main__":
    results = []

    for run in range(N_RUNS):
        print(f"GA2 run {run+1}/{N_RUNS}")
        results.append(run_ga2())
